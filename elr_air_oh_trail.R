...
write.csv(wl_short, "out/ELR1-R1_blend_cf.csv")
...
p_wl <- plot_ly(wl[as.numeric(datetime) %% 10 == 0],
                x = ~datetime,
                y = ~head_cf, #or value, value_adj, value_m, head_masl, etc
                color = ~port,
                colors = viridis(16),
                name = ~portloc,
                type = "scatter", mode = "lines")

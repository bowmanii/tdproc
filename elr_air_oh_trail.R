...
# **** this might not be right, is there actually data here? might just be air monit if there is
...
pr <- pr[datetime %between% c(airtrim_start_well4, airtrim_end_well4)]
...
p_air <- plot_ly(air,
  ...
  layout(
    title = list(text = "ELR1-R2: Air Monitoring",
                 y = 0.98,
                 font = list(size = 18)),

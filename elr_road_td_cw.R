...
cw_e4_sub <- cw_e4_outliers_dd[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start1, cw_pump_end1)]
# remove larger dt
...
cw_e3_sub <- cw_e3[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start1, cw_pump_end1)]
...
cw_e1_sub <- cw_e1_outliers[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start1, cw_pump_end1)]
...
s9 <- subplot(p_wl, p_cw_e4, p_cw_e3, p_cw_e1, shareX = TRUE, nrows = 4, heights = c(0.55, 0.15, 0.15, 0.15))%>%
  layout(
    title = list(text = "ELR2-R1: Temporary Deployment", #Air Corr, Manual Corr, No Corr
                 y = 0.98,
                 font = list(size = 18)),
    xaxis = list(title = "Date and time",
                 nticks = 20,

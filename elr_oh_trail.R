...
pr <- pr[datetime %between% c(blend_start_well1, blend_end_well1)]
...
p_wl <- plot_ly(wl[as.numeric(datetime) %% 10 == 0],
                 x = ~datetime,
                 y = ~head_masl, #or value, value_adj, value_m, head_masletc
                 color = ~port,
                 colors = viridis(16),
                 name = ~portloc,
...
s1 <- subplot(p_wl, p_baro_liner, shareX = TRUE, nrows = 2, heights = c(0.8, 0.2))%>%
  layout(
    title = list(text = "ELR1-R1: Open Hole Monitoring",
                 y = 0.98,
                 font = list(size = 18)),

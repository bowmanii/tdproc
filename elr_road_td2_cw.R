...
cw_e4_sub <- cw_e4_outliers_dd[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start2, cw_pump_end2)]
# remove larger dt
...
cw_e3_sub <- cw_e3[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start2, cw_pump_end2)]
cw_e3 <- NULL
...
cw_e1_sub <- cw_e1_outliers[, .(datetime_utc, flow_hrly_avg, drawdown)][datetime_utc %between% c(cw_pump_start2, cw_pump_end2)]
cw_e1 <- NULL
...
s9 <- subplot(p_wl, p_cw_e4, p_cw_e3, p_cw_e1, shareX = TRUE, nrows = 4, heights = c(0.55, 0.15, 0.15, 0.15))%>%
  layout(
    title = list(text = "ELR2-R2: Temporary Deployment", #Air Corr, Manual Corr, No Corr
                 y = 0.98,
                 font = list(size = 18)),
    xaxis = list(title = "Date and time",
                 nticks = 20,
...
s10 <- subplot(p_wl, p_cw_e4_dd, p_cw_e3_dd, p_cw_e1_dd, shareX = TRUE, nrows = 4, heights = c(0.55, 0.15, 0.15, 0.15))%>%
  layout(
    title = list(text = "ELR2-R2: Temporary Deployment", #Air Corr, Manual Corr, No Corr
                 y = 0.98,
                 font = list(size = 18)),
...
s11 <- subplot(p_wl, scw_flow, scw_dd, shareX = TRUE, nrows = 3, heights = c(0.4, 0.3, 0.3))%>%
  layout(
    title = list(text = "ELR2-R2: Temporary Deployment", #Air Corr, Manual Corr, No Corr
                 y = 0.98,
                 font = list(size = 18)),
